...
async def test_draw_success_saves_reading() -> None:
    ...
    with (
        patch("bot.tarot.run_card_of_day", new=AsyncMock(return_value=trace)),
        patch("bot.tarot.get_trace_hash", return_value="hash-abc"),
        patch("bot.tarot.save_reading", new=AsyncMock()) as mock_save,
        patch("bot.tarot.LLM_MODEL", "test-model"),
        patch("bot.tarot.TAROT_SALT", "salt"),
    ):
...
async def test_draw_exception_handled() -> None:
    with (
        patch("bot.tarot.run_card_of_day", new=AsyncMock(side_effect=RuntimeError("boom"))),
        patch("bot.tarot.save_reading", new=AsyncMock()) as mock_save,
        patch("bot.tarot.LLM_MODEL", "test-model"),
        patch("bot.tarot.TAROT_SALT", "salt"),
    ):
...
async def test_full_reading_success_free() -> None:
    ...
    with (
        patch("bot.tarot.get_user", new=AsyncMock(return_value=(42, "user", None, 2))),
        patch("bot.tarot.run_full_reading", new=AsyncMock(return_value=trace)),
        patch("bot.tarot.get_trace_hash", return_value="hash-xyz"),
        patch("bot.tarot.save_reading", new=AsyncMock()) as mock_save,
        patch("bot.tarot.LLM_MODEL", "test-model"),
    ):
...
async def test_full_reading_suspended_saves_pending_and_invoice() -> None:
    ...
    with (
        patch("bot.tarot.get_user", new=AsyncMock(return_value=(42, "user", None, 0))),
        patch("bot.tarot.run_full_reading", new=AsyncMock(return_value=trace)),
...
async def test_pre_checkout_valid() -> None:
    ...
    from bot.payment import process_pre_checkout_query
...
async def test_successful_payment_resumes_and_saves() -> None:
    ...
    with (
        patch("bot.payment.get_pending_execution", new=AsyncMock(return_value=("tid-001", trace_json))),
        patch("bot.payment.delete_pending_execution", new=AsyncMock()) as mock_delete,
        patch("bot.payment.save_reading", new=AsyncMock()) as mock_save,
        patch("bot.payment.get_trace_hash", return_value="hash-resumed"),
        patch("nano_vm.models.Trace.model_validate_json", return_value=MagicMock()),
        patch("bot.payment.resume_full_reading", new=AsyncMock(return_value=resumed)),
        patch("bot.payment.LLM_MODEL", "test-model"),
    ):
        from bot.payment import process_successful_payment
...
async def test_successful_payment_no_pending() -> None:
    ...
    with (
        patch("bot.payment.get_pending_execution", new=AsyncMock(return_value=None)),
        patch("bot.payment.save_reading", new=AsyncMock()) as mock_save,
    ):
        from bot.payment import process_successful_payment
        await process_successful_payment(msg)
...
async def test_successful_payment_resume_failed() -> None:
    ...
    with (
        patch("bot.payment.get_pending_execution", new=AsyncMock(return_value=("tid-001", trace_json))),
        patch("bot.payment.delete_pending_execution", new=AsyncMock()),
        patch("bot.payment.save_reading", new=AsyncMock()) as mock_save,
        patch("nano_vm.models.Trace.model_validate_json", return_value=MagicMock()),
        patch("bot.payment.resume_full_reading", new=AsyncMock(return_value=resumed)),
        patch("bot.payment.LLM_MODEL", "test-model"),
